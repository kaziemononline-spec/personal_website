...
def test_all():
    ...
    with sync_playwright() as p:
        ...
        page.evaluate("document.querySelector('.service-link[data-service]').click()")
        time.sleep(0.5)
        ...
        check(is_hidden, "Back-to-top hidden at top (opacity/visibility)")
        ...
        nav_hidden = page.evaluate("""() => {
            ...
            return style.right === '-100%' || style.right === '' || nav.getBoundingClientRect().left >= window.innerWidth;
        }""")
        check(nav_hidden, "Nav menu hidden initially on mobile (off-screen)")
        ...
        if nav_links_mobile.count() > 0:
            ...
            time.sleep(0.5)
            nav_closed = page.evaluate("""() => {
                const nav = document.querySelector('.nav-menu');
                const style = getComputedStyle(nav);
                return style.right === '-100%' || nav.getBoundingClientRect().left >= window.innerWidth;
            }""")
            check(nav_closed, "Nav menu closes after link click")
        ...
        # 20. Check page has valid schema.org JSON-LD
        print("\n=== STRUCTURED DATA ===")
